    ...
    return (
        <div className="w-full space-y-4 animate-fade-in pb-10">
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700">
                <h2 className="text-lg font-black text-gray-800 dark:text-white mb-3 text-center flex items-center justify-center gap-2">
                ...
                <div className="mb-4">
                    <div className="flex justify-between items-end mb-1">
                        <span className="text-xl font-black text-orange-600">{totalCaloriesToday.toLocaleString()}<span className="text-[10px] text-gray-400 font-normal ml-1">kcal</span></span>
                        <span className="text-gray-400 text-[9px] font-black uppercase tracking-widest">เป้าหมาย: {tdeeGoal.toLocaleString()}</span>
                    </div>
                ...
                <div className="space-y-4 bg-gray-50 dark:bg-gray-900/30 p-4 rounded-xl border border-gray-100 dark:border-gray-800">
                    <div className="space-y-2">
                        {customImagePreview ? (
                            <div className="space-y-2 animate-fade-in">
                                ...
                                <button type="button" onClick={handleScanWithAI} disabled={isScanning} className="w-full py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-indigo-200 dark:shadow-none hover:shadow-xl transition-all active:scale-95">
                            ...
                            >
                                ...
                                <div className="relative">
                                    <div className="w-16 h-16 bg-white dark:bg-gray-800 rounded-full shadow-xl border-4 border-indigo-50 dark:border-indigo-900 flex items-center justify-center relative z-10 group-hover:scale-105 transition-transform duration-300">
                                        <CameraIcon className="w-8 h-8 text-indigo-500 dark:text-indigo-400" />
                                    </div>
                                    <div className="absolute -right-3 -bottom-1 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white px-2 py-0.5 rounded-full shadow-lg border border-white dark:border-gray-800 flex items-center gap-1 animate-bounce z-20">
                                        <SparklesIcon className="w-2.5 h-2.5" />
                                        <span className="text-[8px] font-black tracking-wider">AI</span>
                                    </div>
                                </div>

                                {/* Text Instructions */}
                                <div className="text-center relative z-10">
                                    <h3 className="text-sm font-black text-indigo-900 dark:text-indigo-100 mb-0.5 group-hover:text-indigo-700 transition-colors">
                                        ถ่ายภาพอาหาร
                                    </h3>
                                    <div className="mt-1 inline-flex items-center gap-1 px-2 py-0.5 bg-white dark:bg-gray-800 rounded text-[8px] text-gray-400 font-bold uppercase tracking-wide border border-indigo-100 dark:border-indigo-900">
                                        JPG/PNG
                                    </div>
                                </div>
                            </button>
                    ...
                    <form onSubmit={handleCustomAdd} className="space-y-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label className="block text-[9px] font-black text-gray-400 uppercase mb-1 ml-1">ชื่ออาหาร</label>
                                <input type="text" value={customName} onChange={(e) => setCustomName(e.target.value)} placeholder="เช่น ข้าวกะเพราไก่..." className="w-full px-3 py-2 bg-white dark:bg-gray-800 border-2 border-gray-100 dark:border-gray-700 rounded-xl text-xs font-bold focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20 outline-none transition-all"/>
                            </div>
                            <div>
                                <label className="block text-[9px] font-black text-gray-400 uppercase mb-1 ml-1">แคลอรี่ (kcal)</label>
                                <input type="number" value={customCalories} onChange={(e) => setCustomCalories(e.target.value)} placeholder="0" className="w-full px-3 py-2 bg-white dark:bg-gray-800 border-2 border-gray-100 dark:border-gray-700 rounded-xl text-xs font-bold focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20 outline-none transition-all"/>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2 bg-white dark:bg-gray-800 p-2 rounded-lg border border-gray-100 dark:border-gray-700">
                            <div>
                                <label className="block text-[8px] font-black text-emerald-600 dark:text-emerald-400 uppercase mb-0.5 text-center">โปรตีน (g)</label>
                                <input type="number" value={customProtein} onChange={(e) => setCustomProtein(e.target.value)} placeholder="0" className="w-full px-1 py-1.5 text-center bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-md text-xs font-bold text-emerald-700 dark:text-emerald-300 focus:ring-1 focus:ring-emerald-500 outline-none"/>
                            </div>
                            <div>
                                <label className="block text-[8px] font-black text-amber-600 dark:text-amber-400 uppercase mb-0.5 text-center">คาร์บ (g)</label>
                                <input type="number" value={customCarbs} onChange={(e) => setCustomCarbs(e.target.value)} placeholder="0" className="w-full px-1 py-1.5 text-center bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-md text-xs font-bold text-amber-700 dark:text-amber-300 focus:ring-1 focus:ring-amber-500 outline-none"/>
                            </div>
                            <div>
                                <label className="block text-[8px] font-black text-rose-600 dark:text-rose-400 uppercase mb-0.5 text-center">ไขมัน (g)</label>
                                <input type="number" value={customFat} onChange={(e) => setCustomFat(e.target.value)} placeholder="0" className="w-full px-1 py-1.5 text-center bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-800 rounded-md text-xs font-bold text-rose-700 dark:text-rose-300 focus:ring-1 focus:ring-rose-500 outline-none"/>
                            </div>
                        </div>

                        {isHealthyChoice && (
                            <div className="p-2 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg flex items-center gap-2 animate-fade-in">
                                <StarIcon className="w-4 h-4 text-emerald-500" />
                                <span className="text-[10px] font-black text-emerald-700 dark:text-emerald-400">มื้อนี้เพื่อสุขภาพ (Bonus HP x2)</span>
                            </div>
                        )}
                        
                        {errorMsg && <div className="p-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg text-[10px] font-bold border border-red-100 dark:border-red-800 flex items-center gap-2"><InformationCircleIcon className="w-3 h-3"/>{errorMsg}</div>}

                        <button type="submit" disabled={!customName || isScanning} className="w-full bg-orange-500 hover:bg-orange-600 text-white font-black py-3 rounded-xl shadow-md transition-all active:scale-[0.98] uppercase text-[10px] tracking-widest disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-sm font-black text-gray-800 dark:text-white uppercase tracking-tight">รายการวันนี้</h3>
                    <button onClick={() => { setItemToDelete(null); setShowConfirmDialog(true); }} className="text-[9px] font-black text-gray-400 hover:text-red-500 uppercase flex items-center gap-1 transition-colors">
                    ...
                    {todaysEntries.length > 0 ? (
                        todaysEntries.map((entry) => (
                            <div key={entry.id} className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-700 flex gap-3 items-center transition-transform hover:scale-[1.01]">
                                {entry.image ? (
                                    <div className="w-12 h-12 rounded-lg overflow-hidden border border-white dark:border-gray-600 flex-shrink-0 bg-black shadow-sm">
                                        <img src={entry.image} alt="Food" className="w-full h-full object-cover" />
                                    </div>
                                ) : (
                                    <div className="w-12 h-12 rounded-lg bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center text-xl flex-shrink-0 border border-orange-200 dark:border-orange-800">🍱</div>
                                )}
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-bold text-gray-800 dark:text-white truncate text-xs">{entry.name}</h4>
                                    <div className="flex flex-wrap items-center gap-2 mt-0.5">
                                        <span className={`text-[10px] font-black ${entry.isHealthyChoice ? 'text-emerald-500' : 'text-orange-500'}`}>{entry.calories} kcal</span>
                                        {(entry.protein || entry.carbs || entry.fat) ? (
                                            <span className="text-[8px] text-gray-400 font-medium bg-white dark:bg-gray-800 px-1 py-0.5 rounded border border-gray-200 dark:border-gray-600">
                                                P{entry.protein || 0} C{entry.carbs || 0} F{entry.fat || 0}
                                            </span>
                                        ) : null}
                                    </div>
                                </div>
                                <button onClick={() => { setItemToDelete(entry.id); setShowConfirmDialog(true); }} className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-colors">
                        ...
                        <div className="text-center py-6 opacity-30 flex flex-col items-center">
                            ...
                            <p className="text-xs font-bold text-gray-400">ไม่มีข้อมูลวันนี้</p>
                        </div>
                    )}
                </div>
            </div>
