...
import { HomeIcon, BellIcon, UserGroupIcon, PhoneIcon, BeakerIcon, BoltIcon, ExclamationTriangleIcon, XIcon, SparklesIcon, MenuIcon, UserCircleIcon, WaterDropIcon, HeartIcon } from './components/icons';
...
const DataSyncGuard: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const { isSyncing, syncError, retrySync, useOfflineData, currentUser, isDataSynced, userPin, isPinVerified } = useContext(AppContext);

    if (!currentUser || currentUser.role === 'guest') return <>{children}</>;

    // Background Sync Logic: If locked by PIN, don't show spinner, but allow children (App content) to mount underneath
    const isLocked = userPin && !isPinVerified;
...
const AppContent: React.FC = () => {
  const { activeView, setActiveView, theme, setTheme, currentUser, logout, userProfile, setUserProfile, waterHistory, calorieHistory, activityHistory, moodHistory, sleepHistory, setWaterHistory, gainXP, isSOSOpen, closeSOS, showLevelUp, closeLevelUpModal, isDataSynced, exitSimulationMode, userPin, isPinVerified, unlockApp } = useContext(AppContext);
  ...
  useEffect(() => {
      // Logic: Only show modals if PIN is verified to avoid stacking modals on top of PIN screen
      if (isDataSynced && currentUser && currentUser.role === 'user' && isPinVerified) {
          const hasSeenOnboarding = localStorage.getItem('has_seen_onboarding_v1');
          
          if (!userProfile.pdpaAccepted) { 
              setShowPDPA(true); 
              setShowOrgModal(false); 
              setShowOnboarding(false);
          } 
          else if (!userProfile.organization || userProfile.organization === '') { 
  ...
  }, [currentUser, userProfile, isDataSynced, isPinVerified]);
  ...
  const SimulationBanner = () => {
      ...
      return (
          <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 z-[100] w-[90%] max-w-sm animate-bounce-in">
              <div className="bg-indigo-900/90 backdrop-blur-md text-white px-4 py-3 rounded-2xl shadow-2xl flex items-center justify-center gap-4 border-2 border-indigo-400/50">
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'dark bg-gray-900 text-gray-100' : 'bg-slate-100 text-slate-900'} font-sans pb-10`}>
      <ErrorBoundary>
        {currentUser ? (
            <>
                {/* --- PIN LOCK OVERLAY (Moved Outside Guard) --- */}
                {userPin && !isPinVerified && (
                    <PinLock onUnlock={unlockApp} />
                )}
                ...
                <DataSyncGuard>
                    ...
                    {isDataSynced && showOnboarding && !showPDPA && !showOrgModal && <OnboardingModal onClose={handleCloseOnboarding} />}
